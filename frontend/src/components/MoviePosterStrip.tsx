...
export default function MoviePosterStrip() {
  const { setSelectedMovie, setCurrentPage, t, trendingMovies, loadTrendingMovies, language } = useAppStore();
  ...
  // Duplicate to create seamless loop (only if we have movies)
  const movies = trendingMovies.length > 0 ? [...trendingMovies, ...trendingMovies] : [];
